...
const FacultyRegistration = () => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (!faceDescriptor) {
            return toast.warning('Please scan the faculty face first.');
        }

        try {
            setLoading(true);
            await axios.post('/api/faculty/register', { ...formData, face_encoding: faceDescriptor }, config());

            toast.success('Faculty Registered Successfully!');
            setFormData({ faculty_id: '', name: '', email: '', college: '', department: '' });
    ...
    return (
        <div className="max-w-4xl mx-auto space-y-6 mb-20">
            <div className="text-center mb-8">
                <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Register Faculty Face Profile</h1>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 flex-1">
                    <h3 className="text-lg font-semibold text-gray-800 dark:text-white mb-4 flex items-center"><UserPlus className="w-5 h-5 mr-2" /> Faculty Details</h3>
                    <form onSubmit={handleSubmit} className="space-y-4">
                        <div>
                            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Faculty ID</label>
                            <input type="text" name="faculty_id" required value={formData.faculty_id} onChange={handleChange} className="w-full px-4 py-2 mt-1 border rounded-md dark:bg-gray-700 dark:border-gray-600 dark:text-white focus:ring-indigo-500 focus:border-indigo-500" placeholder="e.g. F-2001" />
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Name</label>
                            <input type="text" name="name" required value={formData.name} onChange={handleChange} className="w-full px-4 py-2 mt-1 border rounded-md dark:bg-gray-700 dark:border-gray-600 dark:text-white focus:ring-indigo-500 focus:border-indigo-500" placeholder="Jane Smith" />
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Email Address</label>
                            <input type="email" name="email" required value={formData.email} onChange={handleChange} className="w-full px-4 py-2 mt-1 border rounded-md dark:bg-gray-700 dark:border-gray-600 dark:text-white focus:ring-indigo-500 focus:border-indigo-500" placeholder="jane@faculty.edu" />
                        </div>
